...
6. **安全地继续**：可逐岗检查沟通入口，也可编辑统一打招呼文字并授权未投递岗位自动投递。
...
先完成意向确认并确保岗位库中已有岗位，再使用首页「自动投递未处理岗位」。当前仅支持 BOSS 直聘。
...
3. 可在「补充判定规则」明确豁免学历、经验、工作年限等条件，例如“茶行业相关岗位都可投递，不看学历和工作年限”；岗位方向和明确排除职位仍会核验。规则会保存在当前浏览器。
4. BOSS 地区搜索只限定到城市；即使意向填写安溪、晋江等区县，也只使用其所属城市编码，区域和商圈保持不限。
5. 自动投递完整复用单岗位投递顺序：先准备并核验沟通入口，再确认发送；发送时在已核验的 BOSS 编辑器中按 Enter，若页面未响应则兼容点击可见发送按钮，并以输入框清空和新增“送达/已读”回执判定成功。
6. 如需同时发送简历图片，勾选选项并填写**运行服务电脑上的绝对路径**。支持 PNG/JPG/JPEG，文件不得超过5MB；未勾选则不发图片。
7. 勾选「我已核对文字及当前意向，确认向匹配岗位真实发送」，点击「开始自动投递」。**这是实际发送授权，不是预览。**
8. 查看已检查数量、已发送数量和当前处理状态；检查明细按“明确不符合”“无法判定”和其他结果分区，每区按需加载，并可打开原岗位 JD。需要中止时点击「停止后续投递」。

批次规则与边界：

- 候选岗位不限制首次采集或最近发现日期；同平台尚未成功投递的历史岗位也会进入候选。候选清单在启动时固定，运行中新增岗位不加入本批次。
- 使用本次固定的已确认意向重新匹配 JD；只处理明确建议投递的岗位，跳过历史已投递、不符合及待核实岗位。
- 匹配证据会保存岗位 ID、JD 内容版本、意向版本、补充规则版本、结论和置信度。只有条件全部未变化的高置信度“明确不符合”结论会直接复用；无法判定或规则变化时会重新判断。
