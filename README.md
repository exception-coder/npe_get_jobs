...
4. 如需同时发送简历图片，勾选选项并填写**运行服务电脑上的绝对路径**。支持 PNG/JPG/JPEG，文件不得超过5MB；未勾选则不发图片。
5. 勾选「我已核对文字及当前意向，确认向匹配岗位真实发送」，点击「开始自动投递」。**这是实际发送授权，不是预览。**
6. 查看已检查数量、已发送数量和当前处理状态；检查明细按“明确不符合”“无法判定”和其他结果分区，每区按需加载，并可打开原岗位 JD。需要中止时点击「停止后续投递」。
