...
## 2026-09-22 补充判定规则支持明确豁免

- 任务：修复用户写明“不看学历和工作年限”后仍被判为学历或经验不足的问题。
- 根因：模型提示词禁止规则覆盖 JD 明确条件，且后端证据校验只接受 JD 原文，会把引用用户规则的满足结论重新降为无法判定。
- 关键设计决策：学历、经验等准入维度可由用户明确豁免并引用规则原文；岗位方向和明确排除职位仍必须由 JD 证据核验。

## 2026-09-22 BOSS 搜索固定为城市级

- 任务：修复安溪、晋江等意向被转换为具体区县筛选，导致 BOSS 搜索结果过窄的问题。
- 根因：地区解析器同时返回父城市编码和 `areaBusiness` 区县编码，URL 构造器原样发送两个参数。
- 关键设计决策：区县名称只用于反查父城市；URL 构造器无条件忽略 `areaBusiness`，区域和商圈始终不限。

## 2026-09-22 自动投递复用确认发送动作

- 任务：修复自动投递把预设文字填入 BOSS 输入框后没有触发发送的问题。
- 根因：sidecar 在已经校验 `confirmContact=true` 后，又依赖附件选项中的 `sendGreeting` 才点击发送，形成重复且可能不一致的发送开关。
- 关键设计决策：`confirmContact` 是文字发送的唯一授权；单岗位和自动投递统一点击可见发送按钮，并以输入框清空及新增送达/已读回执确认成功。

## 2026-09-23 BOSS 编辑器使用 Enter 真实发送

- 任务：修复自动投递在真实 BOSS 会话中已填入自定义招呼语、发送按钮已启用，但文字仍停留在编辑器的问题。
- 根因：浏览器回归夹具把按钮点击直接模拟为送达，未覆盖 BOSS 当前以编辑器 Enter 触发发送的实际交互。
- 关键设计决策：核验收件人后聚焦编辑器并按 Enter；只有编辑器未清空时才点击发送按钮后备，最后仍以编辑器清空和新增送达/已读回执作为成功依据。

## 2026-09-23 自动投递复用单岗位准备流程

- 任务：修复自动投递与岗位库单岗位“投递”实际调用顺序不一致的问题。
- 根因：单岗位投递在确认前执行 `prepareContact` 并要求入口就绪，自动批次却从 `startFromJob` 直接调用 `confirmContact`。
- 关键设计决策：自动批次逐岗位复用 `prepareContact → confirmContact`；入口未返回 `READY` 时记录明确失败原因且不发送。
