    ...
    public void init() {
        ...
        if (chatModels.isEmpty()) {
            ...
            chatModels.forEach((name, model) -> {
                String status = getModelStatus(name);
                log.info("  {} {} ({})", status, name, model.getClass().getSimpleName());
            });
        }
        ...
        if (!properties.getIncludedModels().isEmpty()) {
            log.info("───────────────────────────────────────────────────────────");
            log.info("仅检查以下模型:");
            properties.getIncludedModels().forEach(name -> 
                log.info("  ✓ {}", name));
        }
        ...
        if (!properties.getExcludedModels().isEmpty()) {
            log.info("───────────────────────────────────────────────────────────");
            log.info("排除以下模型:");
            properties.getExcludedModels().forEach(name -> 
                log.info("  ✗ {}", name));
        }
