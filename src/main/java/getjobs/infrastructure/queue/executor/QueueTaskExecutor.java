...
public class QueueTaskExecutor {
     ...
     */
    private final BlockingQueue<getjobs.infrastructure.queue.domain.QueueTask> taskQueue;
     ...
     * 当前正在执行的任务
     */
    private volatile getjobs.infrastructure.queue.domain.QueueTask currentTask;
     ...
     * @return 队列任务实体
     */
    public getjobs.infrastructure.queue.domain.QueueTask submit(QueueTask queueTask) {
        QueueTaskConfig config = queueTask.getConfig();
        config.validate();

        // 创建任务实体
        getjobs.infrastructure.queue.domain.QueueTask task = getjobs.infrastructure.queue.domain.QueueTask
                .builder()
     ...
     */
    public getjobs.infrastructure.queue.domain.QueueTask submitAndWait(QueueTask queueTask, long timeoutMs)
            throws InterruptedException, TimeoutException {
        getjobs.infrastructure.queue.domain.QueueTask task = submit(queueTask);
    ...
    private void run() {
        ...
        while (running.get() || !taskQueue.isEmpty()) {
            try {
                // 从队列中取出任务（阻塞等待）
                getjobs.infrastructure.queue.domain.QueueTask task = taskQueue.poll(1, TimeUnit.SECONDS);
     ...
     * @param task 任务实体
     */
    private void executeTask(getjobs.infrastructure.queue.domain.QueueTask task) {
     ...
     * @return 队列任务实例
     */
    private QueueTask getQueueTaskFromDomain(getjobs.infrastructure.queue.domain.QueueTask task) {
     ...
     * 计算任务执行耗时（毫秒）
     */
    private long calculateDuration(getjobs.infrastructure.queue.domain.QueueTask task) {
     ...
     * 获取当前正在执行的任务
     */
    public getjobs.infrastructure.queue.domain.QueueTask getCurrentTask() {
        return currentTask;
    }
