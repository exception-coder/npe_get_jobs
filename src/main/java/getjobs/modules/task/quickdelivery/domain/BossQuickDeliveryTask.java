...
public class BossQuickDeliveryTask implements ScheduledTask {
    ...
    public TaskConfig getTaskConfig() {
        return TaskConfig.builder()
                ...
                .globalUnique(true) // 全局唯一，同一时刻只能执行一个Boss直聘投递任务
    ...
    public void afterExecute(boolean success) {
        if (success) {
            log.info("Boss直聘快速投递任务执行成功");
        } else {
            log.error("Boss直聘快速投递任务执行失败，请检查日志");
        }
    }
}
