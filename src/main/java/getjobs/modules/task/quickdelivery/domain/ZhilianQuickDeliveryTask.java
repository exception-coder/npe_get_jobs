...
public class ZhilianQuickDeliveryTask implements ScheduledTask {
    ...
    public TaskConfig getTaskConfig() {
        return TaskConfig.builder()
                ...
                .globalUnique(true) // 全局唯一，同一时刻只能执行一个智联招聘投递任务
    ...
    public void afterExecute(boolean success) {
        if (success) {
            log.info("智联招聘快速投递任务执行成功");
        } else {
            log.error("智联招聘快速投递任务执行失败，请检查日志");
        }
    }
}
