  ...
  <div class="editor-pane">
    <section class="editor-section identity">
      ...
      <div class="form-grid">
        ...
        <label><span>工作经验</span><select v-model="draft.personalInfo.experience"><option value="">请选择</option><option v-for="item in optionsWithCurrent(EXPERIENCE_OPTIONS, draft.personalInfo.experience)" :key="item" :value="item">{{ item }}</option></select></label>
    ...
    <ResumeRepeatableSection eyebrow="05 / 项目经历" title="展示最能代表你的项目" item-label="项目" empty-text="添加 3–5 个与目标岗位最相关的项目。" :items="draft.projects" @add="addProject" @remove="draft.projects.splice($event, 1)">
      <template #default="{ index }"><div class="form-grid"><label><span>项目名称</span><input v-model="draft.projects[index].name" /></label><label><span>项目角色</span><input v-model="draft.projects[index].role" /></label><ResumePeriodField v-model="draft.projects[index].period" label="项目时间" /><label class="full"><span>项目概述</span><textarea v-model="draft.projects[index].summary" rows="3" /></label><label class="full"><span>成果亮点（每行一条）</span><textarea :value="lines(draft.projects[index].highlights)" @input="draft.projects[index].highlights = lineList($event)" rows="5" /></label></div></template>
    </ResumeRepeatableSection>
    ...
    <ResumeRepeatableSection eyebrow="06 / 教育经历" title="保持简明准确" item-label="教育" empty-text="添加学历信息。" :items="draft.education" @add="addEducation" @remove="draft.education.splice($event, 1)">
      <template #default="{ index }"><div class="form-grid"><label><span>学校</span><input v-model="draft.education[index].school" /></label><label><span>专业</span><input v-model="draft.education[index].major" /></label><label class="full"><span>学历</span><select v-model="draft.education[index].degree"><option value="">请选择</option><option v-for="item in optionsWithCurrent(DEGREE_OPTIONS, draft.education[index].degree)" :key="item" :value="item">{{ item }}</option></select></label><ResumeYearRangeField v-model="draft.education[index].period" label="就读时间" /></div></template>
    </ResumeRepeatableSection>
  </div>
</template>
...
import ResumePeriodField from './ResumePeriodField.vue';
import ResumeRepeatableSection from './ResumeRepeatableSection.vue';
import ResumeYearRangeField from './ResumeYearRangeField.vue';

const EXPERIENCE_OPTIONS = ['应届生', ...Array.from({ length: 20 }, (_, index) => `${index + 1} 年`), '20 年以上'];
const DEGREE_OPTIONS = ['初中及以下', '中专 / 中技', '高中', '大专 · 全日制', '大专 · 非全日制', '本科 · 全日制', '本科 · 非全日制', '硕士 · 全日制', '硕士 · 非全日制', '博士 · 全日制', '博士 · 非全日制'];
...
const optionsWithCurrent = (options: string[], current: string) => current && !options.includes(current) ? [current, ...options] : options;
...
.editor-pane { min-width: 0; }.editor-section { border-top: 1px solid var(--line); padding: 28px 0; }.identity { border-top: 0; }.section-heading { margin-bottom: 18px; }.section-heading span { color: var(--accent); font-size: 9px; font-weight: 750; letter-spacing: .1em; }.section-heading h3 { margin: 5px 0 0; color: var(--ink-strong); font: 560 20px var(--font-display); }.form-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 14px; }.form-grid .full { grid-column: 1 / -1; }label { display: grid; gap: 7px; }label > span { color: var(--ink-muted); font-size: 10px; font-weight: 650; }input,select,textarea { width: 100%; border: 1px solid var(--line); border-radius: 8px; background: var(--surface); padding: 10px 12px; color: var(--ink-strong); font-size: 12px; line-height: 1.55; transition: border-color var(--motion-fast), box-shadow var(--motion-fast); }select { min-height: 40px; cursor: pointer; }textarea { resize: vertical; }input:focus,select:focus,textarea:focus { border-color: var(--accent); outline: 0; box-shadow: 0 0 0 3px rgb(138 93 54 / 9%); }
