          ...
          <template v-else>
            ...
            <div v-if="snapshot.jobs.length" class="job-cards">
              <article v-for="job in snapshot.jobs" :key="job.platformJobId" class="job-card">
                <div class="job-card-main"><span>{{ platformName }}</span><h4>{{ job.title || '未命名岗位' }}</h4><p>{{ job.company || '未知公司' }} · {{ job.city || '地点待确认' }}</p><div class="job-tags"><span v-if="job.salary">{{ job.salary }}</span><span>符合当前筛选</span></div></div>
                <div class="job-reason"><span>为什么值得看</span><p>{{ job.description || '岗位信息符合当前关键词与基础筛选条件，建议打开详情进一步判断。' }}</p></div>
                <div class="job-actions"><a :href="job.href" target="_blank" rel="noreferrer">查看详情 <i class="mdi mdi-arrow-top-right" /></a></div>
              </article>
            </div>
...
.result-area { max-width: 900px; margin: 72px auto 0; }.searching-state { display: flex; align-items: center; justify-content: center; gap: 18px; min-height: 180px; }.search-orbit { display: grid; width: 54px; height: 54px; place-items: center; border-radius: 50%; background: var(--selection); color: var(--accent); animation: breathe 1.8s ease-in-out infinite; }.searching-state h3 { margin: 0; color: var(--ink-strong); font: 560 20px var(--font-display); }.searching-state p { margin: 5px 0 0; color: var(--ink-faint); font-size: 11px; }.result-heading { display: flex; align-items: end; justify-content: space-between; gap: 20px; margin-bottom: 24px; }.result-heading span, .empty-result > span { color: var(--accent); font-size: 9px; font-weight: 750; letter-spacing: .1em; }.result-heading h3, .empty-result h3 { margin: 6px 0 0; color: var(--ink-strong); font: 560 27px var(--font-display); letter-spacing: -.02em; }.process-button { border: 0; background: transparent; color: var(--ink-faint); font-size: 10px; cursor: pointer; }.process-strip { display: flex; flex-wrap: wrap; gap: 17px; margin: -7px 0 22px; color: var(--ink-faint); font-size: 10px; }.process-strip i { color: var(--success); }.job-cards { display: grid; gap: 12px; }.job-card { display: grid; grid-template-columns: minmax(0, 1.1fr) minmax(0, 1fr) auto; gap: 30px; align-items: center; padding: 24px; border-radius: 14px; background: var(--surface); box-shadow: 0 4px 20px rgb(58 48 40 / 5%); }.job-card-main > span, .job-reason > span { color: var(--ink-faint); font-size: 9px; font-weight: 700; letter-spacing: .08em; }.job-card h4 { margin: 6px 0 5px; color: var(--ink-strong); font-size: 16px; }.job-card-main p, .job-reason p { margin: 0; color: var(--ink-muted); font-size: 10px; line-height: 1.55; }.job-tags { display: flex; gap: 6px; margin-top: 12px; }.job-tags span { border-radius: 10px; background: var(--surface-subtle); padding: 4px 8px; color: var(--ink-muted); font-size: 9px; }.job-actions a { display: inline-flex; min-height: 36px; align-items: center; gap: 5px; color: var(--accent); font-size: 10px; font-weight: 700; text-decoration: none; }.empty-result { padding: 44px 0; text-align: center; }.empty-result p { color: var(--ink-muted); font-size: 11px; }.empty-result button { margin-top: 10px; }.empty-result.error { color: var(--danger); }.decision-bar { position: sticky; z-index: 20; bottom: 18px; display: flex; align-items: center; justify-content: space-between; gap: 20px; margin-top: 18px; padding: 15px 18px; border-radius: 14px; background: var(--ink-strong); color: white; box-shadow: var(--shadow-elevated); }.decision-bar div { display: grid; gap: 3px; }.decision-bar strong { font-size: 11px; }.decision-bar span { color: rgb(255 255 255 / 62%); font-size: 9px; }.decision-bar button { min-height: 36px; border: 0; border-radius: 18px; background: white; padding: 0 14px; color: var(--ink-strong); font-size: 10px; font-weight: 750; cursor: pointer; }
