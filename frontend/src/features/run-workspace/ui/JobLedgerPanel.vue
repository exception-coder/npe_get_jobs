<template>
  <section class="ledger" aria-labelledby="ledger-title">
    ...
    <template v-else>
      <div class="job-list">
        <article v-for="job in jobs" :key="job.id"><div class="job-core"><span>{{ platformTitle }} · {{ statusLabel(job.status) }}</span><h4>{{ job.jobTitle }}</h4><p>{{ job.companyName || '未知公司' }} · {{ job.workCity || '地点待确认' }}</p></div><div class="job-meta"><strong>{{ job.salaryRange || '薪资面议' }}</strong><span>{{ job.aiMatched === true ? '符合当前偏好' : job.aiMatched === false ? '未通过匹配' : '等待判断' }}</span></div><a v-if="job.jobUrl" :href="job.jobUrl" target="_blank" rel="noreferrer" aria-label="查看岗位详情"><i class="mdi mdi-arrow-top-right" /></a></article>
      </div>
