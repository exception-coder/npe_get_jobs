<template>
  <section class="ledger" aria-labelledby="ledger-title">
    ...
    <form class="filters" @submit.prevent="load(0)"><label><i class="mdi mdi-magnify" /><input v-model="keyword" placeholder="搜索岗位或公司" /></label><select v-model="status" aria-label="岗位状态"><option value="">全部状态</option><option value="history">历史已投递 / 已联系</option><option value="0">待处理</option><option value="1">待联系</option><option value="2">已过滤</option><option value="4">联系失败</option></select><button type="submit">筛选</button></form>
    <p class="history-hint">已投递或已联系的岗位会自动跳过后续投递。在平台手动投递过的岗位，可在下方补记。</p>
    ...
    <div v-else-if="!jobs.length" class="state"><strong>{{ status || keyword ? '没有符合条件的岗位' : `${platformTitle} 还没有岗位` }}</strong><span>{{ status || keyword ? '调整筛选条件后重试。' : '完成一次寻找后，结果会自动出现在这里。' }}</span></div>
    <template v-else>
      <div class="job-list">
        <article v-for="job in jobs" :key="job.id">
          ...
          <div class="job-meta">
            <strong>{{ job.salaryDesc || job.salaryRange || '薪资面议' }}</strong>
            <span :class="matchTone(job)">{{ matchLabel(job) }}</span>
            <span v-if="hasHistory(job)">已投递 / 已联系 · 后续自动跳过</span>
            <button type="button" :disabled="hasHistory(job) || contactBusy || marking !== null"
              @click="emit('contact', job.id)">{{ hasHistory(job) ? '已投递' : contactBusy ? '正在准备…' : '投递' }}</button>
            <button v-if="job.status !== 3" type="button" :disabled="marking !== null"
              @click="markContacted(job, !job.isContacted)">
              {{ marking === job.id ? '正在保存…' : job.isContacted ? '撤回补记' : '补记为已投递' }}
            </button>
            <span v-else>投递成功记录不可撤回</span>
          </div>
          <JobDecisionFacts
...
const emit = defineEmits<{ 'update:platform': [value: PlatformCode]; contact: [jobId: string] }>();
...
function hasHistory(job: JobRecord) { return job.status === 3 || job.isContacted === true; }
async function markContacted(job: JobRecord, contacted: boolean) {
  marking.value = job.id;
  error.value = '';
  try {
    const result = await updateContacted(job.id, contacted);
    if (!result.success) throw new Error('未能更新：岗位不存在或已确认投递成功，请刷新后查看');
    await load(page.value);
  } catch (reason) { error.value = reason instanceof Error ? reason.message : '更新投递标记失败，请重试'; }
...
async function load(target = 0) {
  ...
  error.value = '';
  try {
    const result = await fetchJobRecords({ platform: props.platform, page: target, size, ...(keyword.value.trim() ? { keyword: keyword.value.trim() } : {}), ...(status.value === 'history' ? { contactedOnly: true } : status.value ? { status: Number(status.value) } : {}) });
...
function matchLabel(job: JobRecord) { return hasHistory(job) ? '历史记录命中' : job.aiMatched === true ? '符合当前偏好' : job.aiMatched === false ? '未通过匹配' : '等待判断'; }
...
.history-hint { color: var(--ink-muted); font-size: 12px; line-height: 1.6; margin: 0 0 18px; }
.job-meta button { border: 1px solid var(--line); border-radius: var(--radius-control); padding: 6px 10px; background: var(--surface-subtle); color: var(--ink); cursor: pointer; }
